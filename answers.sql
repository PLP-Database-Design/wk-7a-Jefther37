...
-- The Products column has multiple values (Laptop, Mouse) — we need to separate them.

-- First, create a new table structure for 1NF:
...
-- The table has a partial dependency (CustomerName depends only on OrderID).
...
-- Step 2: Create a separate OrderProducts table:
